...
public:	
	...
	*/
	void removeCharacter() {
		...
		if (numElements == 0) {
			throw std::exception("System cannot delete the first position because the LinkedList is empty");
		}
	...
	void removeCharacter(int position)
	{
		...
		if (numElements == 0) {
			throw std::exception("System cannot delete a position from an empty LinkedList");
		}
