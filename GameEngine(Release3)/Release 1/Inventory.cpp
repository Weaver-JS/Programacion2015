...
Inventory::Inventory()
{
	...
	for (int i = 0; i < inventorySlots.getLenght(); i++) {
		if (i == 0) {
		inventorySlots.dataArray[i].setInitialValues(INVENTORY_X_POSITION, HEIGHT +INVENTORY_Y_POSITION - 20, SPRITE_EMTPY_OBJECT, INVENTORY_WIDTH, INVENTORY_HEIGHT, 0, 1);
		}
		else
		{
			
			inventorySlots.dataArray[i].setInitialValues(INVENTORY_X_POSITION+ INVENTORY_WIDTH* i + xSep   , INVENTORY_Y_POSITION - 20, SPRITE_EMTPY_OBJECT, INVENTORY_WIDTH , INVENTORY_HEIGHT , 0, 1);
