...
SDLInterface::SDLInterface() 	{	
	...
	if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
		...
		//Initializ fonts from SDL_ttf. Return 0 if it is successful
	if (TTF_Init() == 0){
		g_font = TTF_OpenFont("../sharedResources/fonts/Minecraft.ttf", 25);
		if (g_font == 0){
...
void SDLInterface::drawText(std::string text, int backgroundColor, int foregroundColor, int x, int y) {		
	...
	textSurface = TTF_RenderText_Blended(g_font, text.c_str(), colorPalette[backgroundColor]);
